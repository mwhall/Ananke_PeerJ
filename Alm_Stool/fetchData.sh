...
#Fetch each of the noted FASTQ files, if it is labelled :StoolXXX where XXX > 26a
awk 'BEGIN{FS="\t";}{if (NR>1) {split($3, x, ":Stool"); if (x[2] > 261) {split($2, f, ";"); system("wget " f[1]); system("wget " f[2]);}}}' MANIFEST.txt
...
#Get only donor B, and remove ERR531802 because that day was sequenced twice and that's the lower yield sample
# ERR531943 has no sequences remaining after it's run through DADA2, so remove it from metadata
grep 'DonorB\|SampleID' METADATA.txt | grep -v 'ERR531802' | grep -v 'ERR531943' > METADATA_B.txt

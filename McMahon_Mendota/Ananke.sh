...
mprof run ananke import_from_dada2 -i sequence_data/dada2_omega1e2.csv -m sequence_data/METADATA_modified.txt -f seq.unique.fasta -t time_points -o Mendota_Ananke.h5

mprof run --include-children ananke cluster -i Mendota_Ananke.h5 -l 0.01 -u 1 -s 0.01 -d sts -n 4

#Optional: use the TaxAss.sh script to generate the taxonomic classifications
mprof run ananke add taxonomy -i Mendota_Ananke.h5 -d TaxAss/taxonomy.txt
...
mprof run ananke add sequence_clusters -i Mendota_Ananke.h5 -d seq_renumbered.txt
